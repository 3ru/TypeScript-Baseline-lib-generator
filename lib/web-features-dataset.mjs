...
export async function buildWebFeaturesDataset(options) {
    ...
    if (!webFeaturesData.features || typeof webFeaturesData.features !== "object" || Array.isArray(webFeaturesData.features)) {
    ...
    for (const [featureId, feature] of Object.entries(webFeaturesData.features)) {
        ...
        if (feature.compat_features !== undefined && !Array.isArray(feature.compat_features)) {
            throw new Error(`${packageName} feature ${featureId} has non-array compat_features`);
        }
        const rawCompatFeatures = feature.compat_features ?? [];
        if (rawCompatFeatures.some(
            /** @param {unknown} compatKey */
            compatKey => typeof compatKey !== "string",
        )) {
            throw new Error(`${packageName} feature ${featureId} has a non-string compat_features entry`);
        }
...
/**
 * @param {{ repoRoot: string; packageName?: string; dataset: any; }} options
 */
export async function verifyWebFeaturesDataset(options) {
    const expected = await buildWebFeaturesDataset({
        repoRoot: options.repoRoot,
        packageName: options.packageName,
        snapshotDate: options.dataset.snapshot.baselineDate,
        snapshotName: options.dataset.snapshot.name,
    });
    const actual = {
        snapshot: options.dataset.snapshot,
        featureRows: options.dataset.featureRows,
        compatRows: options.dataset.compatRows,
    };
