...
function createTypedArrayFamilyResolver(libCompatRows, baselineTarget) {
    ...
    return () => {
        ...
        const compatRowByKey = new Map(
            libCompatRows.map(compatRow => [compatRow.compatKey, compatRow]),
        );
        /** @type {string[]} */
        ...
        for (const symbol of TYPED_ARRAY_INSTANCE_SYMBOLS) {
            const compatRow = compatRowByKey.get(`javascript.builtins.${symbol}`);
            if (!compatRow) {
                missingSymbols.push(symbol);
                continue;
            }
            if (isCompatIncluded(compatRow, baselineTarget)) {
...
export async function classifyManifest(options) {
    ...
    const managedDerivedRoots = [
        path.join(repoRoot, "derived", "current"),
        path.join(repoRoot, ".tmp"),
    ];
    const classificationOutputPath = resolveManagedOutputPath(
        manifest.classificationOutput,
        repoRoot,
        manifestPath,
        "classificationOutput",
        managedDerivedRoots,
    );
    const compatManagementOutputPath = resolveManagedOutputPath(
        manifest.compatManagementOutput,
        repoRoot,
        manifestPath,
        "compatManagementOutput",
        managedDerivedRoots,
    );
 ...
 * }} options
 */
export function classifyCompatRowsForTarget(options) {
    const typedArrayFamily = createTypedArrayFamilyResolver(options.libCompatRows, options.baselineTarget);
    const classifiedCompatRows = options.libCompatRows.map(compatRow => classifyCompatRow({
        compatRow,
        baselineTarget: options.baselineTarget,
        inventory: options.inventory,
        compatManagementRegistry: options.compatManagementRegistry,
        typedArrayFamily,
    }));
    validateDeclarationMappingCoverage({
        classifiedCompatRows,
        compatManagementRegistry: options.compatManagementRegistry,
        inventory: options.inventory,
        typedArrayFamily,
    ...
    return classifiedCompatRows;
}

/**
 * A declaration mapping opts its root container into reverse-provenance
 * auditing. Every member in that container must be claimed by at least one
 * compat row, so a new TypeScript-only alias cannot be promoted implicitly by
 * complete-container generation.
 *
 * @param {{
 ...
 * }} options
 */
function validateDeclarationMappingCoverage(options) {
    const mappedEntries = options.compatManagementRegistry.entries.flatMap(entry =>
        entry.declarationMapping ? [{ entry, declarationMapping: entry.declarationMapping }] : []
    );
    if (!mappedEntries.length) {
        return;
    }

    const classifiedRowByCompatKey = new Map(
        options.classifiedCompatRows.map(row => [row.compatKey, row]),
    );
    const claimedMemberKeys = new Set(
        options.classifiedCompatRows
            ...
            .flatMap(unit => unit?.ownerSymbol && unit.memberName
                ? [getMemberClaimKey(unit.ownerSymbol, unit.memberName)]
                : []),
    );
    const auditedContainerSymbols = new Set();

    for (const { entry, declarationMapping } of mappedEntries) {
        const classifiedRow = classifiedRowByCompatKey.get(entry.compatKey);
        if (!classifiedRow) {
            throw new Error(`Declaration mapping ${entry.compatKey} has no classified compat row`);
        }
        const rootResolution = resolveCompatRoot(
            options.inventory,
            getCompatSegments(entry.compatKey),
            entry,
            options.typedArrayFamily,
        );
        if (!rootResolution?.rootSurface) {
            throw new Error(`Declaration mapping ${entry.compatKey} has no root surface for ${classifiedRow.compatRoot}`);
        }
        for (const symbol of getDeclarationMappingContainerSymbols(rootResolution.rootSurface, declarationMapping.scope)) {
            auditedContainerSymbols.add(symbol);
        }
    }

    const unclaimedMembers = [...auditedContainerSymbols]
        .flatMap(symbol => options.inventory.memberUnitsByOwnerSymbol.get(symbol) ?? [])
        .flatMap(unit =>
            unit.ownerSymbol
            && unit.memberName
            && !claimedMemberKeys.has(getMemberClaimKey(unit.ownerSymbol, unit.memberName))
                ? [`${unit.ownerSymbol}.${unit.memberName}`]
                : []
        );
    const uniqueUnclaimedMembers = [...new Set(unclaimedMembers)].sort(compareStringsCaseSensitive);
    if (uniqueUnclaimedMembers.length) {
        throw new Error([
            "Declaration mapping coverage drift detected. Members in mapped containers lack compat claims:",
            ...uniqueUnclaimedMembers.map(member => `- ${member}`),
        ].join("\n"));
    }
}
 ...
 */
function getMemberClaimKey(ownerSymbol, memberName) {
    return `${ownerSymbol}\0${memberName}`;
}
...
function classifyCompatRow(options) {
    ...
    if (!rootResolution) {
        return createClassifiedRow(
            ...
            isCompatIncluded(compatRow, baselineTarget),
            {
    ...
    const includeInTarget = isCompatIncluded(compatRow, baselineTarget);
    ...
    if (compatManagement?.declarationMapping) {
        return createClassifiedRow(
            compatRow,
            compatRoot,
            includeInTarget,
            resolveDeclarationMapping({
                compatKey: compatRow.compatKey,
                declarationMapping: compatManagement.declarationMapping,
                inventory,
                rootSurface,
            }),
            compatManagement,
 ...
 * }} options
 */
function resolveDeclarationMapping(options) {
    const containerSymbols = getDeclarationMappingContainerSymbols(
        options.rootSurface,
        options.declarationMapping.scope,
    );
    if (!containerSymbols.length) {
        throw new Error(
            `Declaration mapping ${options.compatKey} found no ${options.declarationMapping.scope} containers`,
        );
    }

    const unitsByContainerAndMember = containerSymbols.flatMap(containerSymbol =>
        options.declarationMapping.memberNames.map(memberName => ({
            containerSymbol,
            memberName,
            units: getMemberUnits(options.inventory, containerSymbol, memberName),
        }))
    );
    const missingMembers = unitsByContainerAndMember
        .filter(entry => !entry.units.length)
        .map(entry => `${entry.containerSymbol}.${entry.memberName}`);
    if (missingMembers.length) {
        throw new Error(
            `Declaration mapping ${options.compatKey} could not resolve ${options.declarationMapping.scope} members: `
                + missingMembers.join(", "),
        );
    }
    ...
    return {
        ...
        resolvedUnitIds: [...new Set(unitsByContainerAndMember.flatMap(entry => entry.units.map(unit => unit.id)))]
            .sort(compareStringsCaseSensitive),
        notes: `${options.compatKey} maps to ${options.declarationMapping.scope} members ${options.declarationMapping.memberNames.join(", ")}`,
        transforms: [],
    };
}
 ...
 * @param {DeclarationMapping["scope"]} scope
 */
function getDeclarationMappingContainerSymbols(rootSurface, scope) {
    const containerSymbols = scope === "static" ? rootSurface.staticContainerSymbols : rootSurface.instanceContainerSymbols;
    const oppositeContainerSymbols = scope === "static" ? rootSurface.instanceContainerSymbols : rootSurface.staticContainerSymbols;
    const ambiguousSymbols = [...containerSymbols]
        .filter(symbol => oppositeContainerSymbols.has(symbol))
        .sort(compareStringsCaseSensitive);
    if (ambiguousSymbols.length) {
        throw new Error(
            `Declaration mapping cannot distinguish ${scope} members on shared containers: ${ambiguousSymbols.join(", ")}`,
        );
    }
...
function classifyConstructorRow(options) {
    ...
    const callableUnits = staticContainerSymbols.flatMap(symbol => [
        ...getPreferredMemberUnits(inventory, symbol, "<construct>"),
        ...getPreferredMemberUnits(inventory, symbol, "<call>"),
    ]);
    const prototypeUnits = staticContainerSymbols.flatMap(symbol =>
        getPreferredMemberUnits(inventory, symbol, "prototype")
    );
    const rootVarUnits = rootSurface.rootDeclarationUnitIds
        .map(unitId => inventory.unitById.get(unitId))
        .filter(isPresent)
        .filter(unit =>
            unit.declarationKind === "var"
            && unit.typeRefName
            && rootSurface.staticContainerSymbols.has(unit.typeRefName)
        );
    const resolvedUnits = qualifierSegments.length
        ? callableUnits
        : [...callableUnits, ...prototypeUnits, ...rootVarUnits];
...
export function resolveRootAvailabilityUnitIds(inventory, rootSurface) {
    const preferredRootUnitIds = new Set(
        getPreferredBaseDeclarationUnits(inventory, rootSurface.compatName).map(unit => unit.id),
    );
    const rootDeclarationUnits = rootSurface.rootDeclarationUnitIds
        .map(unitId => inventory.unitById.get(unitId))
        .filter(isPresent)
        .filter(unit => preferredRootUnitIds.has(unit.id));
    ...
    const preferredStaticSymbols = new Set(
        directRootUnits.flatMap(unit => {
            if (unit.declarationKind === "var") {
                return unit.typeRefName ? [unit.typeRefName] : [];
            }
            return unit.symbolName ? [unit.symbolName] : [];
        }),
    );
    for (const symbol of rootSurface.instanceContainerSymbols) {
        resolvedUnitIds.push(
            ...getPreferredBaseDeclarationUnits(inventory, symbol).map(unit => unit.id),
        );
    }
    for (const symbol of preferredStaticSymbols) {
        resolvedUnitIds.push(
            ...getPreferredBaseDeclarationUnits(inventory, symbol).map(unit => unit.id),
        );
    }
...
/**
 * @param {{ baselineStatus: string | boolean | undefined; baselineLowDate?: string; compatKey: string; }} compatRow
 * @param {string} baselineTarget
 */
function isCompatIncluded(compatRow, baselineTarget) {
    switch (baselineTarget) {
        case "high":
            return compatRow.baselineStatus === "high";
        case "low":
            return compatRow.baselineStatus === "high" || compatRow.baselineStatus === "low";
    }
    const yearMatch = baselineTarget.match(/^year:(\d{4})$/);
    if (!yearMatch) {
        throw new Error(`Unsupported baseline target ${baselineTarget}`);
    }
    if (compatRow.baselineStatus === false) {
        return false;
    }
    ...
    return Number(lowDate.slice(0, 4)) <= Number(yearMatch[1]);
}
