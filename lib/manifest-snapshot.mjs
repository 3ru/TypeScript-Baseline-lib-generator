...
export async function refreshManifestSnapshot(options) {
    ...
    if (updateOutputPaths) {
        manifest.classificationOutput = toPosixRelativePath(repoRoot, path.join(derivedDirectory, "classification.json"));
        manifest.compatManagementOutput = toPosixRelativePath(repoRoot, path.join(derivedDirectory, "compat-management-report.json"));
        manifest.inventoryOutput = toPosixRelativePath(repoRoot, path.join(derivedDirectory, "inventory.json"));
        manifest.generationOutput = toPosixRelativePath(repoRoot, path.join(derivedDirectory, "generation.json"));
    }
