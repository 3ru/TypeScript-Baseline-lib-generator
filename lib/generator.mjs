 ...
 *   compilerSupportUnitIds: Set<string>;
...
export function resolveTypeOnlyDependencyClosure(options) {
    const compatClosure = new Set(resolveDependencyClosure({
        ...
        initiallySelectedUnitIds: new Set([
            ...
            ...options.compilerSupportUnitIds,
        ]),
 ...
 *   }>;
 *   compilerSupportUnitIds: Set<string>;
 *   runtimeAliasUnitIds: Set<string>;
 * }} options
 */
export function assertRuntimeDeclarationProvenance(options) {
    const selectedUnitIds = [...options.selectedUnitIds];
    const emittedUnitIds = collectEmittedUnitIds({
        inventory: options.inventory,
        selectedUnitIds,
        completeContainerUnitIds: options.completeContainerUnitIds,
        excludedUnitIds: options.excludedUnitIds,
    });
    const includedRows = options.classifiedCompatRows.filter(row => row.includeInTarget);
    const claimedUnitIds = new Set(includedRows.flatMap(row => row.resolvedUnitIds));
    const claimedMemberKeys = new Set(
        [...claimedUnitIds]
            .map(unitId => options.inventory.unitById.get(unitId))
            .flatMap(unit => unit?.ownerSymbol && unit.memberName
                ? [`${unit.ownerSymbol}\0${unit.memberName}`]
                : []),
    );
    const runtimeOwnerSymbols = new Set(
        [...options.inventory.rootSurfaceByCompatName.values()]
            .filter(root => root.rootDeclarationUnitIds.some(unitId => {
                const unit = options.inventory.unitById.get(unitId);
                return RUNTIME_DECLARATION_KINDS.has(unit?.declarationKind ?? "");
            }))
            .flatMap(root => [...root.instanceContainerSymbols, ...root.staticContainerSymbols]),
    );
    const compilerSupportUnits = [...options.compilerSupportUnitIds]
        .map(unitId => options.inventory.unitById.get(unitId));
    const runtimeAliasUnits = [...options.runtimeAliasUnitIds]
        .map(unitId => options.inventory.unitById.get(unitId));
    const compilerSupportMemberKeys = new Set(
        [...compilerSupportUnits, ...runtimeAliasUnits].flatMap(unit => unit?.ownerSymbol && unit.memberName
            ? [`${unit.ownerSymbol}\0${unit.memberName}`]
            : []),
    );
    const unprovenUnits = [...emittedUnitIds]
        .map(unitId => options.inventory.unitById.get(unitId))
        .filter(unit => {
            if (
                !unit
                || options.compilerSupportUnitIds.has(unit.id)
                || options.runtimeAliasUnitIds.has(unit.id)
                || claimedUnitIds.has(unit.id)
            ) {
                return false;
            }
            if (unit.unitKind === "member") {
                if (
                    unit.memberName === "<index>"
                    || (unit.ownerSymbol && !runtimeOwnerSymbols.has(unit.ownerSymbol))
                    || (
                        unit.ownerSymbol
                        && unit.memberName
                        && compilerSupportMemberKeys.has(`${unit.ownerSymbol}\0${unit.memberName}`)
                    )
                ) {
                    return false;
                }
                return !unit.ownerSymbol
                    || !unit.memberName
                    || !claimedMemberKeys.has(`${unit.ownerSymbol}\0${unit.memberName}`);
            }
            return RUNTIME_DECLARATION_KINDS.has(unit.declarationKind ?? "")
                && unit.declarationKind !== "namespace"
                && !(
                    options.inventory.fileByLibFileName.get(unit.libFileName)?.preserveWholeFile
                    && !unit.containerPath.length
                );
        })
        .map(unit => unit?.id)
        .filter(unitId => unitId !== undefined)
        .sort(compareStringsCaseSensitive);
    if (unprovenUnits.length) {
        throw new Error(
            "Emitted runtime declarations lack compat or compiler-support provenance:\n"
...
async function createGenerationPlan({ manifestPath, repoRoot }) {
    ...
    if (!manifest.firstClassLib?.yearDirectory) {
        throw new Error(`Manifest ${manifestPath} is missing firstClassLib.yearDirectory`);
    }
    if (!Number.isInteger(manifest.firstClassLib?.firstYear)) {
        throw new Error(`Manifest ${manifestPath} is missing firstClassLib.firstYear`);
    }

    const testOutputRoot = path.join(repoRoot, ".tmp");
    const topLevelOutputPath = resolveManagedOutputPath(
        manifest.firstClassLib.outputFile,
        repoRoot,
        manifestPath,
        "firstClassLib.outputFile",
        [path.join(repoRoot, "generated", "current"), testOutputRoot],
    );
    const allowOutputDirectory = resolveManagedOutputPath(
        manifest.firstClassLib.allowDirectory,
        repoRoot,
        manifestPath,
        "firstClassLib.allowDirectory",
        [path.join(repoRoot, "generated", "current", "allow"), testOutputRoot],
    );
    ...
    const yearOutputDirectory = resolveManagedOutputPath(
        manifest.firstClassLib.yearDirectory,
        repoRoot,
        manifestPath,
        "firstClassLib.yearDirectory",
        [path.join(repoRoot, "generated", "current", "year"), testOutputRoot],
    );
    const managedDerivedRoots = [path.join(repoRoot, "derived", "current"), testOutputRoot];
    const generationOutputPath = resolveManagedOutputPath(
    ...
    const inventoryOutputPath = resolveManagedOutputPath(
        manifest.inventoryOutput,
        repoRoot,
        manifestPath,
        "inventoryOutput",
        managedDerivedRoots,
    ...
    const mainSelection = createTargetSelection({
        inventory,
        classifiedCompatRows: classification.classifiedCompatRows,
        compilerSupportSurfaces: classification.compatManagementRegistry.compilerSupportSurfaces,
        runtimeAliasSurfaces: classification.compatManagementRegistry.runtimeAliasSurfaces,
    });
    const libCompatRows = classification.dataset.compatRows
        .filter(
            /** @param {{ compatKey: string; }} row */
            row => isLibCompatKey(row.compatKey),
        )
        .sort(
            /** @param {{ compatKey: string; }} left @param {{ compatKey: string; }} right */
            (left, right) => compareStringsCaseSensitive(left.compatKey, right.compatKey),
        );
    const yearEntries = resolveBaselineYears(
        manifest.snapshot.baselineDate,
        manifest.firstClassLib.firstYear,
    ).map(year => {
        const classifiedCompatRows = classifyCompatRowsForTarget({
            libCompatRows,
            baselineTarget: `year:${year}`,
            inventory,
            compatManagementRegistry: classification.compatManagementRegistry,
        });
        return {
            year,
            outputPath: path.join(yearOutputDirectory, String(year), "index.d.ts"),
            ...createYearCompatAudit(classifiedCompatRows),
    ...
    const emittedBaselineUnitIds = collectEmittedUnitIds({
        inventory,
        selectedUnitIds: mainSelection.selectedUnitIds,
        completeContainerUnitIds: mainSelection.completeContainerUnitIds,
        excludedUnitIds: mainSelection.excludedUnitIds,
    });
...
/**
 * @param {Array<{ compatKey: string; includeInTarget: boolean; resolutionKind: string; }>} classifiedCompatRows
 */
function createYearCompatAudit(classifiedCompatRows) {
    /** @type {string[]} */
    const includedCompatKeys = [];
    /** @type {string[]} */
    const notModeledUpstreamCompatKeys = [];
    for (const row of classifiedCompatRows) {
        const declarationBacked = DECLARATION_BACKED_RESOLUTION_KINDS.has(row.resolutionKind);
        if (!declarationBacked && !NON_DECLARATION_RESOLUTION_KINDS.has(row.resolutionKind)) {
            throw new Error(`Unknown compat resolution kind ${row.resolutionKind}`);
        }
        if (!row.includeInTarget) {
            continue;
        }
        if (declarationBacked) {
            includedCompatKeys.push(row.compatKey);
        }
        else if (row.resolutionKind === "not-modeled-upstream") {
            notModeledUpstreamCompatKeys.push(row.compatKey);
        }
    }
    return {
        includedCompatKeys: includedCompatKeys.sort(compareStringsCaseSensitive),
        notModeledUpstreamCompatKeys: notModeledUpstreamCompatKeys.sort(compareStringsCaseSensitive),
    };
}
 ...
 *   }>;
 *   compilerSupportSurfaces: string[];
 *   runtimeAliasSurfaces: string[];
 * }} options
 */
function createTargetSelection(options) {
    const compatSelectedUnitIds = options.classifiedCompatRows
        .filter(row => row.includeInTarget)
        .flatMap(row => row.resolvedUnitIds);
    const { excludedUnitIds, excludedRowsByUnitId } = resolveExcludedUnits(options.classifiedCompatRows);
    const typeOnlyUnitIds = resolveUnclaimedTypeOnlyUnitIds({
        inventory: options.inventory,
        classifiedCompatRows: options.classifiedCompatRows,
        excludedUnitIds,
    });
    const registrySupportUnitIds = resolveCompilerSupportSurfaceUnitIds(
        options.inventory,
        options.compilerSupportSurfaces,
    );
    const runtimeAliasUnitIds = resolveRuntimeAliasUnitIds(
        options.inventory,
        options.runtimeAliasSurfaces,
    );
    const excludedRegistrySupportUnitIds = registrySupportUnitIds.filter(unitId => excludedUnitIds.has(unitId));
    if (excludedRegistrySupportUnitIds.length) {
        throw new Error(
            "Compiler support surfaces conflict with excluded compat declarations:\n"
    ...
    const initiallySelectedUnitIds = new Set([
        ...
        ...typeOnlyUnitIds,
        ...registrySupportUnitIds,
    ]);
    const compilerSupportUnitIds = new Set(registrySupportUnitIds);
    const registrySupportContainerUnitIds = registrySupportUnitIds.filter(unitId => {
        const unit = options.inventory.unitById.get(unitId);
        return unit?.unitKind === "declaration" && Boolean(unit.containerId);
    });
    const completeContainerUnitIds = new Set([
        ...options.classifiedCompatRows
        ...
        ...registrySupportContainerUnitIds,
    ]);
    for (const unitId of collectEmittedUnitIds({
        inventory: options.inventory,
        selectedUnitIds: registrySupportUnitIds,
        completeContainerUnitIds: new Set(registrySupportContainerUnitIds),
        excludedUnitIds,
    })) {
        compilerSupportUnitIds.add(unitId);
    }
    ...
    const selectedBeforeCompilerStabilization = resolveTypeOnlyDependencyClosure({
        inventory: options.inventory,
        compatSelectedUnitIds: new Set(compatSelectedUnitIds),
        compilerSupportUnitIds: new Set(registrySupportUnitIds),
        typeOnlyUnitIds,
    ...
    const selectedUnitIds = stabilizeCompilerGlobalSupport({
        inventory: options.inventory,
        unitTextOverrides,
        selectedUnitIds: selectedBeforeCompilerStabilization,
        completeContainerUnitIds,
        excludedUnitIds,
    });
    ...
    assertRuntimeDeclarationProvenance({
        ...
        excludedUnitIds,
        classifiedCompatRows: options.classifiedCompatRows,
        compilerSupportUnitIds,
        runtimeAliasUnitIds: new Set(runtimeAliasUnitIds),
    });
    return {
        selectedUnitIds,
        completeContainerUnitIds,
 ...
 */
function resolveRuntimeAliasUnitIds(inventory, surfaces) {
    return surfaces.flatMap(surface => {
        const units = inventory.units.filter(
            unit => unit.unitKind === "member" && unit.surfacePath === surface,
        );
        if (!units.length) {
            throw new Error(`Runtime alias surface is not modeled by the TypeScript lib inventory: ${surface}`);
        }
 ...
 * @param {string[]} surfaces
 */
function resolveCompilerSupportSurfaceUnitIds(inventory, surfaces) {
    return surfaces.flatMap(surface => {
        const memberUnits = inventory.units.filter(
            unit => unit.unitKind === "member" && unit.surfacePath === surface,
        );
        const units = memberUnits.length
            ? memberUnits
            : getPreferredDeclarationUnits(inventory, surface).filter(
                unit => !RUNTIME_DECLARATION_KINDS.has(unit.declarationKind ?? ""),
            );
        if (!units.length) {
            throw new Error(`Compiler support surface is not modeled by the TypeScript lib inventory: ${surface}`);
        }
        return units.map(unit => unit.id);
    }).sort(compareStringsCaseSensitive);
}
 ...
 */
export function resolveBaselineYears(baselineDate, firstYear) {
    const match = baselineDate.match(/^(\d{4})-\d{2}-\d{2}$/);
    const timestamp = Date.parse(`${baselineDate}T00:00:00Z`);
    if (
        !match
        || !Number.isFinite(timestamp)
        || new Date(timestamp).toISOString().slice(0, 10) !== baselineDate
    ) {
        throw new Error(`Invalid Baseline snapshot date ${baselineDate}`);
    }
    const snapshotYear = Number(match[1]);
    if (!Number.isInteger(firstYear) || firstYear < 2015 || firstYear >= snapshotYear) {
        throw new Error(`Invalid first Baseline year ${firstYear}`);
    }
    return Array.from({ length: snapshotYear - firstYear }, (_, index) => index + firstYear);
}
...
async function publishGenerationPlan(plan) {
    ...
    for (const previousOutputPath of previousOutputs) {
        if (!nextOutputs.has(previousOutputPath)) {
            await removeManagedPath(
                previousOutputPath,
                plan.repoRoot,
                [path.join(plan.repoRoot, "generated", "current"), path.join(plan.repoRoot, ".tmp")],
            );
        }
    }
    ...
    await removeManagedPath(
        plan.allowOutputDirectory,
        plan.repoRoot,
        [path.join(plan.repoRoot, "generated", "current", "allow"), path.join(plan.repoRoot, ".tmp")],
        { recursive: true },
    );
    ...
    await removeManagedPath(
        plan.yearOutputDirectory,
        plan.repoRoot,
        [path.join(plan.repoRoot, "generated", "current", "year"), path.join(plan.repoRoot, ".tmp")],
        { recursive: true },
    );
    for (const entry of plan.yearEntries) {
        const contents = emitSelectedUnits({
            inventory: plan.inventory,
            selectedUnitIds: entry.selectedUnitIds,
            unitTextOverrides: entry.unitTextOverrides,
            completeContainerUnitIds: entry.completeContainerUnitIds,
            excludedUnitIds: entry.excludedUnitIds,
        });
        entry.contentHash = hashText(contents);
        await mkdir(path.dirname(entry.outputPath), { recursive: true });
        await writeFile(entry.outputPath, contents);
    }
...
function getGenerationReport(plan) {
    return {
        ...
        summary: {
            ...
            yearLibCount: plan.yearEntries.length,
        },
        ...
        yearEntries: plan.yearEntries.map(entry => ({
            year: entry.year,
            outputPath: formatPathForReport(plan.repoRoot, entry.outputPath),
            contentHash: entry.contentHash,
            includedCompatKeys: entry.includedCompatKeys,
            notModeledUpstreamCompatKeys: entry.notModeledUpstreamCompatKeys,
            selectedUnitCount: entry.selectedUnitIds.length,
            preservedTypeOnlyUnitCount: entry.typeOnlyUnitIds.length,
            transformedUnitCount: entry.unitTextOverrides.size,
        })),
        ...
        for (const entry of previousReport.outputEntries ?? []) {
            ...
            previousOutputEntries.push(
                resolveManagedOutputPath(
                    ...
                    repoRoot,
                    generationOutputPath,
                    "outputEntries[].outputPath",
                    [path.join(repoRoot, "generated", "current"), path.join(repoRoot, ".tmp")],
                ),
            );
        }
 ...
 *     outputPath: string;
 *   }>;
 *   yearOutputDirectory: string;
 *   yearEntries: Array<{
 *     year: number;
 *     outputPath: string;
 *     contentHash?: string;
 *     includedCompatKeys: string[];
 *     notModeledUpstreamCompatKeys: string[];
 *     selectedUnitIds: string[];
 *     completeContainerUnitIds: Set<string>;
 *     excludedUnitIds: Set<string>;
 *     excludedRowsByUnitId: Map<string, string[]>;
