...
async function createGenerationPlan({ manifestPath, repoRoot }) {
    ...
    const yearOutputDirectory = path.resolve(path.dirname(manifestPath), manifest.firstClassLib.yearDirectory);
    ...
    const yearEntries = resolveBaselineYears(
        ...
        return {
            ...
            ...createTargetSelection({ inventory, classifiedCompatRows }),
        };
    });
...
async function publishGenerationPlan(plan) {
    ...
    await rm(plan.yearOutputDirectory, { recursive: true, force: true });
