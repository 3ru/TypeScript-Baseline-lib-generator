...
export function compareYearContracts(previousReportText, currentReportText) {
    ...
    /** @type {Array<{ year: number; kind: "added" | "removed" | "expanded" | "changed"; requiredVersionBump: "major" | "minor"; }>} */
    const changes = [];
    /** @type {"major" | "minor" | undefined} */
    let requiredVersionBump;

    for (const [year, previousContract] of previousContracts) {
        const currentContract = currentContracts.get(year);
        if (!currentContract) {
            changes.push({ year, kind: "removed", requiredVersionBump: "major" });
        ...
        /** @type {{ year: number; kind: "expanded" | "changed"; requiredVersionBump: "major" | "minor"; }} */
        const change = removedCompatKey || !addedCompatKey
            ? { year, kind: "changed", requiredVersionBump: "major" }
            : { year, kind: "expanded", requiredVersionBump: "minor" };
        changes.push(change);
        if (change.requiredVersionBump === "major" || !requiredVersionBump) {
            requiredVersionBump = change.requiredVersionBump;
        }
    }

    for (const year of currentContracts.keys()) {
        if (!previousContracts.has(year)) {
            changes.push({ year, kind: "added", requiredVersionBump: "minor" });
