...
export const REGEXP_LEGACY_STATIC_MEMBER_NAMES = Object.freeze([
    "$1", "$2", "$3", "$4", "$5", "$6", "$7", "$8", "$9",
    "input", "$_", "lastMatch", "$&", "lastParen", "$+", "leftContext", "$`", "rightContext", "$'",
]);

export const REGEXP_LEGACY_STATIC_ABSENCE_ASSERTION = [
    "type __BaselineLegacyRegExpStatic =",
    ...REGEXP_LEGACY_STATIC_MEMBER_NAMES.map((memberName, index) =>
        `    | ${JSON.stringify(memberName)}${index === REGEXP_LEGACY_STATIC_MEMBER_NAMES.length - 1 ? ";" : ""}`
    ),
...
export const LOW_NEGATIVE_PROBE_CANDIDATES = [
    ...
    {
        compatKey: "javascript.builtins.Iterator.from",
        sourceText: "Iterator.from([1, 2, 3]);",
        errorPattern: /Iterator/,
        absencePattern: /declare abstract class Iterator/,
    },
    {
